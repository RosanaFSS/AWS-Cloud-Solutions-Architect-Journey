...
# Diagnostic Log
echo "Starting startup script!"> /var/log/custom-init.log
...
echo ".container{max-width:800px; margin: 0 auto; background-color: #6e3bb4; padding: 20px; border-radius: 10 px; box-shadow: 0px 4px 6px rgba(0,0,0,0.1);}" >> /var/www/html/index.html
...
if [-z "$AZ"]; then
 ...
 echo "Error: AZ unavailable" >> /var/log/custom-init.log
else
 echo "<p>$AZ</p>" >> /vr/www/html/index.html
 echo "AZ added to HTML" >> /var/log/custom-init.log
fi
...
echo "Finalizing the initialization script " >> /var/log/custom-initi.log
